...
class RegistrationTestMethods(unittest.TestCase):
    ...
    def test_sphere_translation_SSD_full(self):
        ...
        log_var_v = torch.log(torch.ones_like(mu_v))
        u_v = torch.zeros_like(log_var_v)

        var_f = float(0.1 ** 2) * torch.ones(self.dims_im)
        log_var_f = torch.log(var_f).to('cuda:0').requires_grad_(True)
        u_f = torch.zeros_like(log_var_f)
        ...
        optimizer_q_v = Adam([mu_v, log_var_v, u_v], lr=5e-4)
        optimizer_q_f = Adam([log_var_f, u_f], lr=5e-4)
        optimizer_q_phi = Adam(enc.parameters(), lr=1e-4)
        ...
        optimizer_q_f.zero_grad()
        optimizer_q_phi.zero_grad()
        ...
        optimizer_q_f.step()
        optimizer_q_phi.step()

        total_loss += loss_q_f_q_phi.item()
    ...
    def test_sphere_translation_LCC_data_only(self):
        ...
        mu_v = torch.zeros(self.dims_v).to('cuda:0').requires_grad_(True)
        optimizer_q_v = Adam([mu_v], lr=5e-4)
        ...
        for iter_no in range(self.no_steps_v):
            ...
            total_loss += (loss_q_v.item() / float(self.no_steps_v))
