...
class GradTestMethods(unittest.TestCase):
    def setUp(self):
        print(self._testMethodName + '\n')

    def test_grad_v_uniform(self):
    ...
    def test_grad_v_uniform_non_square_grid(self):
        # init. another 3D velocity field
        v = torch.zeros(dims_v_non_square, device=device)
        v[0, 0, ...] = 5.0
        v[0, 1, ...] = 4.0
        v[0, 2, ...] = 2.0
    ...
    def test_grad_v(self):
        ...
        # test that they're correct
        nabla_vx_true = torch.zeros(dims_v, device=device)
        nabla_vy_true, nabla_vz_true = torch.zeros_like(nabla_vx_true), torch.zeros_like(nabla_vx_true)
    ...
    def test_grad_v_non_square_grid(self):
        # init. a uniform 3D velocity field
        v = torch.zeros(dims_v_non_square, device=device)
        ...
        for idx_z in range(v.shape[2]):
            for idx_y in range(v.shape[3]):
                for idx_x in range(v.shape[4]):
                    v[0, 0, idx_z, idx_y, idx_x] = 10.0 * idx_x
                    v[0, 1, idx_z, idx_y, idx_x] = 3.0 * idx_y + 2.5 * idx_z + 0.1
                    v[0, 2, idx_z, idx_y, idx_x] = 0.2 * idx_z
        ...
        # test that they're correct
        nabla_vx_true = torch.zeros(dims_v_non_square, device=device)
        nabla_vy_true, nabla_vz_true = torch.zeros_like(nabla_vx_true), torch.zeros_like(nabla_vx_true)

        nabla_vx_true[:, 0] = 10.0
        nabla_vy_true[:, 1] = 3.0
        nabla_vy_true[:, 2] = 2.5
        nabla_vz_true[:, 2] = 0.2
        ...
        for idx_z in range(displacement.shape[2]):
            for idx_y in range(displacement.shape[3]):
                for idx_x in range(displacement.shape[4]):
                    d = torch.tensor([2.0 * idx_x / (dim_x - 1.0), 2.0 * idx_y / (dim_y - 1.0), 2.0 * idx_z / (dim_z - 1.0)], device=device)
