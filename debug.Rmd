...
from utils import laplacian_1D, laplacian_3D

_s = 7
_lambda = 1.0

L_1D = laplacian_1D(_s)
L_3D = laplacian_3D(_s)
...
"""
calculate the 3D Sobolev kernel
"""

from utils import sobolev_kernel_3D

S = sobolev_kernel_3D(_s, _lambda)
print(S)
```

```{python}
"""
save a structured grid to file
"""

from utils import init_identity_grid_3d, save_grid_to_disk

nx = ny = nz = 128
grid = init_identity_grid_3d(nx, ny, nz)
grid = grid[0].permute(3, 0, 1, 2)

file_path = './grid.vtk'
save_grid_to_disk(grid, file_path)
```
...
from utils import sobolev_kernel_1D
